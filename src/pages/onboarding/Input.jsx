...
const Input = ({ stage, setStage, setInputs, submit }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 mb-5 mx-5">
            {stage !== 'FINISH' && (
                <div>
                    <input
                        ref={input => input && input.focus()}
                        className="pl-[24px] w-full h-[50px] bg-slate-300 rounded-[11px] text-[16px] font-medium font-['Pretendard'] focus:outline-none"
                        placeholder={placeHolder}
                        onChange={onInputChanged}
                        value={inputText}
                    />
                    {isInputEmpty && (
                        <div className="right-[22px] top-[16px] absolute text-white text-[16px] font-medium font-['Pretendard']" onClick={onAnswered}>패스할래요</div>
                    )}
                    {!isInputEmpty && (
